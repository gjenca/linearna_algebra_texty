...
Napríklad
$$ \begin{pmatrix} 1 & -3 & \frac{1}{2} \\ 0 & 4 & -2 \end{pmatrix} \begin{pmatrix} 2 \\ 0 \\ 1 \end{pmatrix} =
\begin{pmatrix} \frac{5}{2} \\ -2 \end{pmatrix} $$
...
V nasledujúcej skupine príkladov máme vždy
\[
\vec x=(x_1,\dots,x_n)
\]
\begin{priklad}[Nulové matice]
Uvažujme nulovú maticu $0$ typu $m \times n$. Aké zobrazenie $[[0]]\colon\mathbb{R}^{n}
\rightarrow \mathbb{R}^{m}$ táto matica reprezentuje?
Počítajme:
$$ 
\underbrace{
...
konštantné zobrazenie s hodnotou $\vec{0}$. Asi nikoho neprekvapí, že toto zobrazenie
sa značí $0$.
\end{priklad}
...
$$
\end{priklad}
\begin{priklad}[skalárny násobok $\vec x\mapsto \alpha.\vec x$]
...
\footnote{
Prvky $\mathbb{R}^1$ sú usporiadané 1-ice; $\mathbb{R}^1 = \mathbb{R}$.
}
...
$$ (\begin{matrix} \frac{1}{n} & \dots & \frac{1}{n} \end{matrix}) \begin{pmatrix} x_1 \\ x_2 \\ \vdots \\ x_n \end{pmatrix} = \frac{x_1}{n} + \dots + \frac{x_n}{n} = \frac{x_1 + \dots + x_n}{n} $$
\end{priklad}
...
Pokúsme sa interpretovať toto zobrazenie geometricky; stotožníme vektory z
$\mathbb{R}^2$ s vektormi v rovine prostredníctvom nejakej zvyčajnej dvojice navzájom kolmých
súradnicových osí s rovnakou mierkou
...
V tomto momente vzniká prirodzená otázka, ktoré zobrazenia $\mathbb{R}^n \rightarrow \mathbb{R}^m$ sú reprezentovateľné
maticami $\R^{m \times n}$;
...
Táto perspektíva nám umožňuje dosiahnuť trochu hlbší vhľad do povahy množín riešení
systémov lineárnych rovníc: sú uzavreté na istý typ lineárnych kombinácií.
\begin{veta}\label{veta:affine}
Nech $\vec{x},\vec{y}\in\R^n$ sú riešenia nejakého systému lineárnych rovníc o $n$
neznámych, nech $c,d\in\R$ sú také skaláry, že $c+d=1$. Potom $c\vec{x}+d\vec{y}$
je tiež riešenie toho systému.
\end{veta}
\begin{proof}
To, že $\vec x,\vec y$ sú riešenia, vieme zapísať $A\vec x=\vec b$ a $A\vec y=\vec
b$, kde $A$ je matica sústavy a $\vec b$ je pravá strana. Pomocou známych vlastností
maticových a vektorových operácií dostaneme
\[
A(c\vec{x}+d\vec{y})=A(c\vec x)+A(d\vec y)=c(A\vec x)+d(A\vec y)=c\vec b+d\vec b=
(c+d)\vec b=1.b=\vec b
\]
čo presne znamená, že $c\vec{x}+d\vec{y}$ je tiež riešením.
\end{proof}

Napríklad zo skúsenosti vieme, že sústava lineárnych rovníc nebude mať presne dve
riešenia. Veta \ref{veta:affine} nám hovorí, prečo to tak je: ak má sústava
lineárnych rovníc viac ako jedno riešenie, má ich nekonečne veľa.
