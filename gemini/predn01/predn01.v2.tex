...
\[ a + bi \]
kde $a, b \in \R$. Tento zápis sa nazýva \textbf{algebrický tvar}.
Platí stotožnenie:
\begin{itemize}
    \item $a \equiv (a, 0)$
    \item $i \equiv (0, 1)$
...
Štruktúra $( \mathbb{C}, +, \cdot, 0, 1 )$ tvorí pole.

\paragraph{Náčrt dôkazu:} Axiómy pre sčítanie sú zrejmé. Pre násobenie: ak $z = a+bi$ a $z \neq 0$, ako vyzerá $z^{-1}$?
...
Je potrebné overiť, že platí: $(a+bi) \cdot z^{-1} = 1$. (DÚ).

Ostatné vlastnosti operácií sčítania a násobenia komplexných čísel je možné overiť priamym výpočtom (čo môže byť prácne, preto to nebudeme vykonávať).
...
\end{center}
\begin{itemize}
    \item Reálna časť $z$ ($\mathrm{Re}\, z$) sa vynáša na os x.
    \item Imaginárna časť $z$ ($\mathrm{Im}\, z$) sa vynáša na os y.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Konjugované číslo} (združené) k číslu $z= a+ib$ je $\overline{z} = a-bi = \mathrm{Re}\, z - (\mathrm{Im}\, z)i$.
\end{itemize}
\end{definicia}
...
Z pravouhlého trojuholníka v komplexnej rovine vyplýva:
\[ \mathrm{Im}\, z = |z| \sin(\arg z) \]
\[ \mathrm{Re}\, z = |z| \cos(\arg z) \]
kde $\arg z$ je uhol, ktorý zviera sprievodič bodu $z$ s kladnou polosou x.
Dosadením dostávame $z = \mathrm{Re}\, z + i \mathrm{Im}\, z = |z|\cos(\arg z) + i|z|\sin(\arg z)$.
Teda
\[ z = |z|(\cos \varphi + i \sin \varphi) \]
kde $\varphi = \arg z$. Tento zápis sa nazýva \textbf{goniometrický tvar} komplexného čísla.
