...
\begin{definicia}
Nech $V, U$ sú vektorové priestory. Hovoríme, že zobrazenie $f: V \to U$ je \textbf{lineárne}, ak platia nasledujúce podmienky:
\begin{enumerate}
...
\end{figure}
    ...
    \end{itemize}
    V oboch prípadoch dostaneme ten istý vektor (pretože rovnobežník na dolnom obrázku je zrotovaný rovnobežník z horného obrázku).
...
\begin{enumerate}[a)]
    \item $f(\vec{0}) = \vec{0}$
    \item Pre každú $n$-ticu skalárov $(a_1, \dots, a_n) \subset \mathbb{R}$ a vektorov $\vec{x}_1, \dots, \vec{x}_n \in V$ platí:
