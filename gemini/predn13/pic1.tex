...
\begin{tikzpicture}[
    ...
    \def\theta{40} % Rotation angle in degrees

    % === Coordinates ===
    \coordinate (O) at (0,0);
    \coordinate (X1_tip) at (\vlen, 0);
    \coordinate (X2_tip) at (0, \vlen);
    \coordinate (LX1) at (\theta:\R);
    \coordinate (LX2) at (90+\theta:\R);

    % === Circle ===
    \draw[thin] (O) circle (\R);

    % === Projections of l_theta(x1) ===
    \coordinate (LX1_x) at ({\R*cos(\theta)}, 0);
    \coordinate (LX1_y) at (0, {\R*sin(\theta)});
    ...
    \node[anchor=east, color=red!80!black] (yc_label) at (-0.3, {\R*sin(\theta)}) {$\vec{x_2} \cdot \sin(\theta)$};
    \draw[->, thin, red!80!black, shorten >=2pt] (yc_label.east) -- (LX1_y);
    
    % Right angle symbol for the projection with a dot
    ...
    % === Angles ===
    % Theta for x1 (blue)
    \draw[angle arc, color=blue!70!cyan] (1.3,0) arc (0:\theta:1.3) node[midway, right, xshift=2pt] {$\theta$};

    % Theta for x2 (red)
    \draw[angle arc, color=red!80!black] (0,1.3) arc (90:90+\theta:1.3) node[midway, above, yshift=2pt] {$\theta$};

    % Theta + pi/2 (green)
    \draw[angle arc, color=green!60!black] (0.8,0) arc (0:90+\theta:0.8);
    \node[color=green!60!black] at (45+\theta:1.3) {$\theta + \frac{\pi}{2}$};

    % Right angle between rotated vectors (green square)
    \coordinate (A) at ($(O)!0.4!(LX1)$);
    \coordinate (B) at ($(O)!0.4!(LX2)$);
    \coordinate (C) at ($(A)+(B)-(O)$);
    \draw[thin, green!60!black] (A) -- (C) -- (B);
