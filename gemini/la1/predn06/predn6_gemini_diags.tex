...
\usepackage{tikz}
\usetikzlibrary{arrows.meta, bending, decorations.pathmorphing}
...
(matica a s ňou asociované zobrazenie stotožníme)
...
$\mathbb{R}^{m \times n} = $ matice typu $M \times M$
$\updownarrow$
reprezentuje zobrazenie
$\updownarrow$
zobrazenia $\mathbb{R}^{n} \rightarrow \mathbb{R}^{n}$

POZOR! nie všetky zobrazenia sú reprezentovateľné maticami.
...
\subsection*{Nulové matice}
Uvažujme nulovú maticu typu $m \times n$. Aké zobrazenie $\mathbb{R}^{n} \rightarrow \mathbb{R}^{m}$ táto matica reprezentuje?
Počítajme:
$$ \begin{pmatrix} 0 & 0 & \dots & 0 \\ 0 & 0 & \dots & 0 \\ \vdots & \vdots & \vdots & \vdots \\ 0 & 0 & \dots & 0 \end{pmatrix} \begin{pmatrix} x_{1} \\ \vdots \\ x_{n} \end{pmatrix} = \begin{pmatrix} 0 \\ \vdots \\ 0 \end{pmatrix} $$
...
\subsection*{Jednotkové matice a ich skalárne násobky}
...
$$ \begin{pmatrix} 1 & 0 & 0 & \dots & 0 \\ 0 & 1 & 0 & \dots & 0 \\ \vdots & \vdots & \vdots & \ddots & \vdots \\ 0 & 0 & 0 & \dots & 1 \end{pmatrix} \begin{pmatrix} a_{1} \\ a_{2} \\ \vdots \\ a_{n} \end{pmatrix} = \begin{pmatrix} a_{1} \\ a_{2} \\ \vdots \\ a_{n} \end{pmatrix} $$
$I_n \in \mathbb{R}^{n \times n}$ \hspace{1em} $\in \mathbb{R}^{n}$ \hspace{1em} $\in \mathbb{R}^{n}$

Vidíme teda, že $I_n \vec{x} = \vec{x}$ pre všetky vektory $\vec{x} \in \mathbb{R}^n$ a teda $I_n$ reprezentuje identické zobrazenie $id_{\mathbb{R}^n}$; $[[I_n]] = id_{\mathbb{R}^n}$.
...
Teda (symbolicky) $D_{\alpha}\vec{x} = \alpha\vec{x},$ pre všetky $\alpha \in \mathbb{R}$ a $\vec{x} \in \mathbb{R}^n$.
Všimnime si, že (keďže $D_0 = 0$ a $D_1 = I_n$) toto dostávame spätne ako špeciálne prípady $\alpha=0, \alpha=1$.

$D_0 = 0$ (nulová matica) \quad $D_0\vec{x} = 0\vec{x} = \vec{0}$
...
\subsection*{Pravouhlá projekcia na priamku}
...
\begin{tikzpicture}[scale=0.8, >={Stealth[length=2mm]}]
    % Osi
    \draw [->, thick] (-3.5,0) -- (4,0) node [right] {$x_1$};
    \draw [->, thick] (0,-3.5) -- (0,4) node [above] {$x_2$};
    
    % Priamka q
    \draw [blue, dashed] (-3,-3) -- (3,3) node [right] {$q$};
    
    % Vektor x = (3,1)
    \draw [->, red, very thick] (0,0) -- (3,1) node [right, black] {$\vec{x}=(3,1)$};
    
    % Vektor P(x) = (2,2)
    \draw [->, red, very thick] (0,0) -- (2,2) node [above, black] {$[[P]](\vec{x})=(2,2)$};
    
    % Projekcia
    \draw [black, dashed] (3,1) -- (2,2);
    
    % Vektor y = (-1,-2)
    \draw [->, green, very thick] (0,0) -- (-1,-2) node [below, black] {$\vec{y}=(-1,-2)$};
    
    % Vektor P(y) = (-1.5, -1.5)
    \draw [->, green, very thick] (0,0) -- (-1.5,-1.5) node [left, black] {$[[P]](\vec{y})=(-\frac{3}{2},-\frac{3}{2})$};
    
    % Projekcia
    \draw [black, dashed] (-1,-2) -- (-1.5,-1.5);
...
Po vyskúšaní pár bodov dospejeme k hypotéze
$[[P]](\vec{x})$ je priemet $\vec{x}$ na $q$ "v pravom uhle". Táto hypotéza je naozaj pravdivá. Je možné ju dokázať "holými rukami", ale rozumnejšie je odložiť jej dôkaz naneskôr, do druhého semestra, keď budeme mať k dispozícii mocnejšie nástroje.

\subsection*{Rovinná rotácia}
Uvažujme, pre $\alpha \in \langle 0, 2\pi \rangle$ maticu $L_{\alpha} = \begin{pmatrix} \cos\alpha & -\sin\alpha \\ \sin\alpha & \cos\alpha \end{pmatrix}$
Máme
...
\begin{tikzpicture}[scale=0.9, >={Stealth[length=2mm]}]
    % Osi
    \draw [->, thick] (-2,0) -- (4,0) node [right] {$x_1$};
    \draw [->, thick] (0,-2) -- (0,4) node [above] {$x_2$};
    ...
    % Originalny vektor
    \coordinate (X) at (0:\veclength); % Start on x-axis for simplicity
    \draw [->, red, very thick] (0,0) -- (\veclength, 0) node [right, black] {$\vec{x}$};
    
    % Rotovany vektor
    \coordinate (LX) at (\myangle:\veclength);
    \draw [->, blue, very thick] (0,0) -- (LX) node [above, black] {$[[L_{\alpha}]](\vec{x})$};
    
    % Arc for angle
    \draw [->, black, dashed] (1.5,0) arc (0:\myangle:1.5) node [midway, right] {$\alpha$};
    
    % Alternativny vektor x (nech nie je na osi)
    \coordinate (X2) at (20:2.5);
    \draw [->, gray, thick, dashed] (0,0) -- (X2) node [right] {$\vec{y}$};
    
    % Rotovany alternativny vektor
    \coordinate (LX2) at (20+\myangle:2.5);
    \draw [->, gray, thick, dashed] (0,0) -- (LX2) node [above] {$[[L_{\alpha}]](\vec{y})$};
    
    % Arc for angle 2
    \draw [->, gray, opacity=0.8] (20:1) arc (20:20+\myangle:1);
...
\subsection*{Osová súmernosť podľa priamky}
...
podľa priamky, ktorú už poznáme
$$ q = \{(t,t) : t \in \mathbb{R}\} $$
...
\begin{tikzpicture}[scale=0.8, >={Stealth[length=2mm]}]
    ...
    \draw [blue, dashed] (-1,-1) -- (4,4) node [right] {$q$};
    
    % Vektor x
    \draw [->, red, very thick] (0,0) -- (3,1) node [right, black] {$\vec{x}=(3,1)$};
    
    % Vektor B(x)
    \draw [->, red, very thick] (0,0) -- (1,3) node [above, black] {$[[B]](\vec{x})=(1,3)$};
    
    % Spojnica
    \draw [black, dashed] (3,1) -- (1,3);
...
\textbf{Veta 7.1} Nech $A \in \mathbb{R}^{m \times n}, B \in \mathbb{R}^{k \times m}$.
Uvažujme zobrazenia reprezentované maticami $A, B$.
$$ [[A]]: \mathbb{R}^n \rightarrow \mathbb{R}^m $$
$$ [[B]]: \mathbb{R}^m \rightarrow \mathbb{R}^k $$
Potom zložené zobrazenie $[[B]] \circ [[A]]$ je reprezentované súčinom matíc $B A$.
...
&= [[B A]](\vec{x}) & \text{(predpis pre $[[B A]]$)}
\end{align*}
Zobrazenia $[[B A]]$ a $[[B]] \circ [[A]]$ajú rovnaký definičný obor aj koobor a každý vektor $\vec{x} \in \mathbb{R}^n$ zobrazia na rovnaký vektor v $\mathbb{R}^k$. To znamená $[[B]] \circ [[A]] = [[B A]]$.

\subsection*{Príklady}
Ak vezmeme maticu rovinnej rotácie $L_{\frac{\pi}{2}}$:
...
$$ L_{\frac{\pi}{2}} L_{\frac{\pi}{2}} = \begin{pmatrix} 0 & -1 \\ 1 & 0 \end{pmatrix} \begin{pmatrix} 0 & -1 \\ 1 & 0 \end{pmatrix} = \begin{pmatrix} -1 & 0 \\ 0 & -1 \end{pmatrix} $$
Táto matica by mala reprezentovať zložené zobrazenie $L_{\frac{\pi}{2}} \circ L_{\frac{\pi}{2}}$. Ale $L_{\frac{\pi}{2}} \circ L_{\frac{\pi}{2}} = L_{\pi}$.
Skontrolujme:
$$ L_{\pi} = \begin{pmatrix} \cos\pi & -\sin\pi \\ \sin\pi & \cos\pi \end{pmatrix} = \begin{pmatrix} -1 & 0 \\ 0 & -1 \end{pmatrix} $$
Pokračujme ďalej a spočítajme $L_{\pi} \cdot L_{\pi}$
$$ L_{\pi} L_{\pi} = \begin{pmatrix} -1 & 0 \\ 0 & -1 \end{pmatrix} \begin{pmatrix} -1 & 0 \\ 0 & -1 \end{pmatrix} = \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix} = I_2 $$
$D_{-1}$ reprezentuje zobrazenie násobenie skalárom $-1$. Ale vynásobiť rovinný vektor skalárom $-1$ je to isté ako otočiť ho o $\pi$ vľavo alebo vpravo, čiže teda $D_{-1} = L_{\pi}$.
V ďalšom kroku by sme mali dostať jednotkovú maticu $I_2$. Porozmýšľajte prečo a presvedčte sa, že to vyjde.

Ak vezmeme pravouhlú projekciu $P$ na priamku $q$ (viď vyššie), očakávame, že $P P = P$, pretože obrazom vektorov (bodov) ležiacich na $q$ sú oni sami. Toto vyjde:
...
\textbf{Definícia 7.2} Nech $A$ je štvorcová matica typu $n \times n$. Inverzná matica k $A$ je taká matica $A^{-1}$ $n \times n$ s vlastnosťou
$$ A A^{-1} = A^{-1} A = I_n $$
Pozor! Inverzná matica k štvorcovej matici nemusí existovať. Napríklad nulová matica iste nemá inverznú, prečo?

\textbf{Definícia 7.3} Nech $A$ je štvorcová matica. Ak $A$ má inverznú, hovoríme, že $A$ je regulárna. V opačnom prípade hovoríme, že $A$ je singulárna.
...
Týmto sme dokázali (alebo to je "Podľa Definície 1.14 z LinAlg 1.3 sme dokázali presne toto"):

\textbf{Veta 7.4} Nech $A \in \mathbb{R}^{n \times n}$ je regulárna matica. Potom $[[A^{-1}]]$ je inverzné zobrazenie k zobrazeniu $[[A]]$.
