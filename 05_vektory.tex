...
Descartes. Zaujímavé je, že v tom čase sa už dlho používali sférické
...
\begin{definicia}[Násobenie geometrického vektora skalárom]
...
\begin{definicia}[násobenie vektora skalarom v $\R^n$]
...
\begin{definicia}[Sčítanie vektorov v $\R^n$]
...
\subsubsection{Vlastnosti vektorových operácií}
...
\end{itemize}
(Poznamenávame, že násobenie skalárom má prednosť pred sčítaním vektorov, podobne ako
pri násobení a sčítaní čísel).

\begin{framed}
Neexistuje nič také ako násobenie vektorov medzi sebou!
\end{framed}

Odčítanie vektorov $\vec{x}-\vec{y}$ je definované ako $\vec{x}-\vec{y} := \vec{x} + (-1)\vec{y}$. Je to teda odvodená operácia zavedená pomocou sčítania a násobenia $(-1)$. Samozrejme, ako ľahko vidieť, odčítanie vektorov prebieha tiež po zložkách:
$$ \vec{x}-\vec{y} = (x_1-y_1, \dots, x_n-y_n) $$
...
\begin{definicia}[Lineárna kombinácia]
