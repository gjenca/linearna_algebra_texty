...
Vieme, že ak uvažujeme rovinu s počiatkom $O$, zavedenie dvoch kolmých súradnicových osí pretínajúcich sa v počiatku s
...
\[ n\varphi_k = \psi + 2k\pi, \quad k \in \{0, 1, \dots, n-1\} \]
Teda
...
\begin{tikzpicture}[>=Stealth, thick,scale=1.4]
    ...
    \tikzset{
        axisline/.style={blue!80!black},
        guide/.style={blue!80!black, densely dotted},
        rootcircle/.style={black}, % User requested black circle
    ...
    % Roots: 45 deg, 165 deg, 285 deg [cite: 119]
    \coordinate (Z0) at (45:\radius);
    \coordinate (Z1) at (165:\radius);
    \coordinate (Z2) at (285:\radius);

    % --- 1. Axes (No labels as requested) ---
    % Using blue style from source [cite: 34, 70]
    \draw[axisline] (-2.5,0) -- (2.5,0);
    \draw[axisline] (0,-2.5) -- (0,2.5);

    % --- 2. Black Circle ---
    \draw[rootcircle] (O) circle (\radius);

    % --- 3. Dot at sqrt(2) ---
    \fill[black] (\radius, 0) circle (1.5pt) node[below right, labeltext] {$\sqrt{2}$};

    % --- 4. Geometric Construction ---
    % Guide lines from origin [cite: 71]
    \draw[guide] (O) -- (Z0);
    \draw[guide] (O) -- (Z1);
    \draw[guide] (O) -- (Z2);
    ...
    % --- 5. Root Points & Labels ---
    % Standard point style from source [cite: 37, 45]
    ...
    % --- 6. Angle Annotation ---
    % Argument arc [cite: 79]
    \draw[red!85!black] (0.5,0) arc (0:45:0.5);
    \node[redtext] at (22.5:0.75) {$\frac{\pi}{4}$};

\end{tikzpicture}
\end{center}
Jedno z riešení rovnice je ,,pekné'' riešenie $1+i$, ostatné dve sú komplikovanejšie.
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
$k$ & \textbf{Goniometrický (polárny) tvar} & \textbf{Algebraický (kartézsky) tvar} \\ \hline
$0$ & $\sqrt{2} \left( \cos \frac{\pi}{4} + i \sin \frac{\pi}{4} \right)$ & $1 + i$ \\ \hline
$1$ & $\sqrt{2} \left( \cos \frac{11\pi}{12} + i \sin \frac{11\pi}{12} \right)$  & $\frac{-1-\sqrt{3}}{2} + i \frac{\sqrt{3}-1}{2}$ \\ \hline
$2$ & $\sqrt{2} \left( \cos \frac{19\pi}{12} + i \sin \frac{19\pi}{12} \right)$  & $\frac{\sqrt{3}-1}{2} - i \frac{\sqrt{3}+1}{2}$ \\ \hline
\end{tabular}
\end{center}
\end{priklad}
